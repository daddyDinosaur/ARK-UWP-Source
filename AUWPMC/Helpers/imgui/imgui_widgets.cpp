...
bool ImGui::tab(const char* icon, const char* label, const ImVec2& size_arg, const bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (selected)
        draw->AddRectFilledMultiColor(ImVec2(bb.Min), ImVec2(bb.Min.x + 1.5f, bb.Max.y), ImColor(109, 84, 90), ImColor(109, 84, 90), ImColor(106, 81, 121), ImColor(106, 81, 121));


    ImGui::PushFont(zzzz);
    window->DrawList->AddText(ImVec2(bb.Min.x + size_arg.x / 2 - ImGui::CalcTextSize(label).x / 2, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(label).y / 2 + 15), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 255.f / 255.f), label);
    ImGui::PopFont();

    ImGui::PushFont(icons);
    window->DrawList->AddText(ImVec2(bb.Min.x + size_arg.x / 2 - ImGui::CalcTextSize(icon).x / 2, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(icon).y / 1.3f), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 255.f / 255.f), icon);
    ImGui::PopFont();
...
bool ImGui::sub(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(270.5f, 19), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    draw->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(30, 28, 31), ImColor(30, 28, 31), ImColor(21, 21, 21), ImColor(21, 21, 21));
    draw->AddRect(bb.Min, bb.Max, ImColor(42, 42, 44));

    if (selected)
    {
        window->DrawList->AddText(ImVec2(bb.Min.x + size.x / 2 - ImGui::CalcTextSize(label).x / 2, bb.Min.y + size.y / 2 - ImGui::CalcTextSize(label).y / 2), ImColor(84 / 255.f, 21 / 255.f, 115 / 255.f, 255.f / 255.f), label);
    }
    else
    {
        window->DrawList->AddText(ImVec2(bb.Min.x + size.x / 2 - ImGui::CalcTextSize(label).x / 2, bb.Min.y + size.y / 2 - ImGui::CalcTextSize(label).y / 2), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 255.f / 255.f), label);
    }
...
bool ImGui::Checkbox2(const char* label, bool* v)
{
    ...
    ImGui::InvisibleButton(label, ImVec2(15 + width, height));
    ...
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(152 / 255.f, 75 / 255.f, 191 / 255.f, 1.f), ImVec4(61 / 255.f, 10 / 255.f, 87 / 255.f, 1.f), t));
    else
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(152 / 255.f, 75 / 255.f, 191 / 255.f, 1.f), ImVec4(61 / 255.f, 10 / 255.f, 87 / 255.f, 1.f), t));

    draw_list->AddRectFilled(ImVec2(p.x + 18, p.y + 3), ImVec2(13 + p.x + width, p.y + height - 3), col_bg, height * 0.5f);
    draw_list->AddCircleFilled(ImVec2(15 + p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - 1.5f, ImColor(255, 255, 255, 255));
    ...
    if (label_size.x > 0.0f)
    {
        if (*v)
        {
            ...
            RenderText(ImVec2((p.x + width) + 20, (p.y + height) - 16), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(220, 30));
    ...
    // Draw frame
    draw->AddRect(ImVec2(total_bb.Min.x, frame_bb.Min.y + 11), ImVec2(total_bb.Max.x, frame_bb.Max.y - 11), ImColor(202, 202, 207), 6);
    draw->AddRectFilled(ImVec2(total_bb.Min.x, frame_bb.Min.y + 11), ImVec2(grab_bb.Min.x + 5, frame_bb.Max.y - 11), ImColor(50, 50, 55), 6);


    // Render grab

    const bool hoveredgrab = ItemHoverable(grab_bb, id);
    ...
    if (hoveredgrab && it_filled->second >= 1.f)
        it_filled->second = ImClamp(it_filled->second - 2.2f * ImGui::GetIO().DeltaTime, 0.5f, 1.f);
    else
        it_filled->second = ImClamp(it_filled->second + (13.2f * ImGui::GetIO().DeltaTime * ((hoveredgrab) ? 1.f : -1.f)), 0.0f, 1.f);

    draw->AddRectFilled(ImVec2(grab_bb.Min.x - 4, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x + 4, grab_bb.Max.y - 6), ImColor(235, 235, 240), 4);
    draw->AddRect(ImVec2(grab_bb.Min.x - 4, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x + 4, grab_bb.Max.y - 6), ImColor(202, 202, 207), 4);
    draw->AddRectFilled(ImVec2(grab_bb.Min.x - 4, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x + 5, grab_bb.Max.y - 5), GetColorU32(ImVec4(242 / 255.f, 242 / 255.f, 247 / 255.f, it_filled->second)), 4);
    draw->AddLine(ImVec2(grab_bb.Min.x + 2, grab_bb.Min.y + 10), ImVec2(grab_bb.Min.x + 2, grab_bb.Max.y - 10), ImColor(50, 50, 55));
    draw->AddLine(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y + 10), ImVec2(grab_bb.Min.x + 5, grab_bb.Max.y - 10), ImColor(50, 50, 55));
    draw->AddLine(ImVec2(grab_bb.Max.x - 2, grab_bb.Min.y + 10), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 10), ImColor(50, 50, 55));
    ...
    draw->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Min.x + 45, frame_bb.Max.y), ImColor(202, 202, 207), 4);
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Min.x + 45, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));


    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + 25, frame_bb.Min.y - 20), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, power, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

    ImColor bodyColor = ImColor(int(24), int(24), int(24), 255);
    // Render grab
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.GetCenter().y), ImVec2(frame_bb.Max.x - 10, frame_bb.GetCenter().y), frame_col);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), ImColor(128, 0, 128));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, ImColor(128, 0, 128));
    }
    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y + 25), value_buf, value_buf_end, NULL, ImVec2(0.5, 0.5));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), label);
    ImGui::NewLine();
